...
export function ExerciseDialog({ isOpen, onOpenChange, exercise, moduleId }: ExerciseDialogProps) {
  return (
    <Dialog 
      open={isOpen} 
      onOpenChange={(open) => {
          ...
          <DialogTitle className="flex items-center gap-3 text-lg sm:text-xl font-bold text-foreground">
            {exercise?.id ? (
              ...
              <>
                ...
                Créer une nouvelle exercice
              </>
            )}
          </DialogTitle>
          <DialogDescription id="dialog-description" className="text-sm text-muted-foreground mt-2">
            {exercise?.id
              ? "Modifiez les détails de la exercise existante pour mettre à jour son contenu."
              : "Créez une nouvelle exercise pour évaluer les connaissances des étudiants."}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="p-6 pt-4">
          {moduleId ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <h3 className="text-lg font-semibold mb-2">Module non spécifiée</h3>
              <p className="text-sm text-muted-foreground max-w-sm">
                Impossible de créer ou modifier un exercice sans un module associée.
              </p>
            </div>
          )}
        </div>
